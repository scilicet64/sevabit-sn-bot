...
from sevabit_sn_bot_config import TELEGRAM_TOKEN, PERSISTENCE_FILENAME, NODE_URL, OWNER
...
def start(bot, update, user_data):
    reply_text = 'Hi!  I can give you sevabit service node information and send you alerts if the uptime proof for your service node(s) gets too long.  I can also optionally let you know when your service nodes earn a payment.\n'
...
def status(bot, update, user_data):
    ...
    reply_text += 'Current SN stake requirement: *{:.2f}* SEVA\n'.format(lsr)
    reply_text += 'Current SN reward: *{:.4f}* SEVA'.format(snbr)
...
def service_node(bot, update, user_data, i, reply_text = '', callback = None):
    ...
    menu = InlineKeyboardMarkup([
        [InlineKeyboardButton('Refresh', callback_data='refresh:{}'.format(i)),
         InlineKeyboardButton('View on sevabitblocks.com', url='https://sevabitblocks.com/service_node/{}'.format(pubkey))],
...
def sevabit_updater():
    ...
    while not time_to_die:
        ...
        try:
            ...
            print("An exception occured during sevabit stats fetching: {}".format(e))
            continue
        last = now
        network_info = status
                ...
                for chatid, user_data in data.items():
                    ...
                    for i in range(len(user_data['sn'])):
                        ...
                        sn_details_buttons = InlineKeyboardMarkup([[
                            ...
                            InlineKeyboardButton('sevabitblocks.com', url='https://sevabitblocks.com/service_node/{}'.format(pubkey))]])
                        if pubkey not in sn_states:
                                ...
                                if my_addrs and len(info['contributors']) > 1:
                                    for y in info['contributors']:
                                        if any(y['address'].startswith(x) for x in my_addrs):
                                            ...
                                            my_rewards.append('*{:.3f} SEVA* (_{}...{}_)'.format(mine, y['address'][0:7], y['address'][-3:]))

                                if send_message_or_shutup(updater.bot, chatid,
                                        '💰 Service node _{}_ earned a reward of *{:.3f} SEVA* at height *{}*.'.format(name, reward, lrbh) + (
...
sevabit_thread = None
def start_sevabit_update_thread():
    global sevabit_thread, network_info
    sevabit_thread = threading.Thread(target=sevabit_updater)
    sevabit_thread.start()
    while True:
        if network_info:
            print("Sevabit data fetched")
            return
        time.sleep(0.25)


def stop_sevabit_thread(signum, frame):
    global time_to_die, sevabit_thread
    time_to_die = True
    sevabit_thread.join()
